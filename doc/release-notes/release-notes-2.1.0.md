Nengcoin core 2.2.0 is rebased off bitcoin/litecoin core 0.13.3
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/ShorelineCrypto/nengcoin/issues>
...
updates will be released anymore. Without security updates, using a nengcoin
...
ECDSA signatures inside Nengcoin transactions now use validation using
...
Previous versions of Nengcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Nengcoin Core was to raise the default minimum
relay fee.

Nengcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Nengcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running nengcoind without having to do any manual configuration.
...
Nengcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Nengcoin Core will relay transactions with
...
entirely in Nengcoin Core 0.13, and it is at this time undecided whether the
...
Nengcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Nengcoin Core automatically creates a hidden service to listen on, without
manual configuration. Nengcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Nengcoin Core is listening, and
...
Nengcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Nengcoin
Core to use floating fees. By default, Nengcoin Core will use floating
fees.
...
(default: `0.0002` NENG/kB).

At all times, Nengcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) NENG.
Furthermore, Nengcoin Core will never create transactions paying less than
...
`nengcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
previously showed an assembly representation of:
    ...
    304502207fa7a6d1e0ee81132a269ad84e68d695483745cde8b541e3bf630749894e342a02.2.0c1f7ab20e13e22fb95281a870f3dcf38d782e53023ee313d741ad0cfbc0c509001 400000 OP_NOP2

now shows as:

    304502207fa7a6d1e0ee81132a269ad84e68d695483745cde8b541e3bf630749894e342a02.2.0c1f7ab20e13e22fb95281a870f3dcf38d782e53023ee313d741ad0cfbc0c5090[ALL] 400000 OP_CHECKLOCKTIMEVERIFY
...
Then, to tunnel a SSL connection on 26376 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 26376 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /nengcoinrpc>
        ProxyPass http://127.0.0.1:6376/
        ProxyPassReverse http://127.0.0.1:6376/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass nengcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< nengcoinrpc:<password>
    </Location>
...
Restarting nengcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in nengcoin.conf
...
nengcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/nengcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Nengcoin Core code base has
...
- `nengcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `nengcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Nengcoin scripting system that in combination with
...
Nengcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Nengcoin Core.
...
The two phases are distinct in the Nengcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Nengcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Nengcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Nengcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of nengcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of nengcoins being spent is exactly
...
- **Increased security for multisig:** Nengcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Nengcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Nengcoin's current ledger (called the UTXO set),
  ...
  note that Nengcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Nengcoin users to individually opt-in to almost any change in the Nengcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Nengcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Nengcoin
client to Nengcoin Core 0.13.2 to signal for segwit support. When a
...
Since Nengcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `nengcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `nengcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
